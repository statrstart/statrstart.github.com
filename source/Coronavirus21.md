...
> 2022/06/28 発表  
> 大阪市保健所管内において、1～３月の死亡が新たに９２件判明した。
...
### 2020/12/1から2021/2/28まで,2021/3/1から2021/6/20まで,2021/6/21から2021/12/31,2022/1/1から現在まで 年代別 重症者数と死亡者数
...
- 第６波は未就学児、就学児、１０代の重症者が今までの波より多い。
...
- 2022/2/14に１００歳代の重症者（大阪府発表）がでました。すべての年代で重症者がでたのは第６波が初めてです。
...
![covid21_1502](https://raw.githubusercontent.com/statrstart/statrstart.github.com/master/source/images/covid21_1502.png)

#### 大阪府：入院、宿泊療養、自宅療養、入院.療養等調整中（2021年2月12日以降）
...
b<-barplot(d,beside=T,legend=T,las=1,col=c("royalblue","orange","brown"),ylim=c(0,max(d)*1.2),
	args.legend=list(x="topleft",inset=0.03,legend=c("2020-12-01から2021-02-28まで","2021-03-01から2021-06-20まで","2021-06-21から2022-08-31まで")))
text(x=b,y=d,labels=d,pos=3)
...
barplot(d ,beside=T,col=c("royalblue","brown"),las=T,legend=T,args.legend =list(x ="topleft",inset=0.03))
title("大阪府：性別＆年代別  死亡者数(2021-06-21から2022-08-31まで)")
par(mfrow=c(1,1))
#dev.off()
```
...
par(mar=c(5,5,5,2))
cols=c("brown3","royalblue3","yellow4","darkolivegreen")
b<- barplot(t(plotdata),col=cols,border=cols,las=1,xaxt="n",legend=T,args.legend=list(x="topleft",inset=0.03))
box(bty="l",lwd=2.5)
axis(1,at=b[grep("01",substring(rownames(Long),9,10))],labels=paste0(c(3:12,1),"月"))
mtext(text="2022年",at=b[324],side=1,line=2.5,cex=1.2)
title("患者の状況（前日24時まで）",cex.main=1.5)
#dev.off()
```
